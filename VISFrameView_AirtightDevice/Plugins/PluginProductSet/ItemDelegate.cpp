...
QWidget *ItemDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (delegateType == LineEditDelegate) {
        ...
        if (index.column() == 1) {
            comboBox->addItems(GlobalParam->recipeMotor.listRecipe);
        }
        else if (index.column() == 2) {
            comboBox->addItems(GlobalParam->recipeTray.listRecipe);
        }
        else if (index.column() == 3) {
            comboBox->addItems(GlobalParam->recipeElectricSaw.listRecipe);
        }
        else if (index.column() == 4) {
            comboBox->addItems(GlobalParam->recipeAirtight.listRecipe);
        }
        return comboBox;
    }
