...
bool ParamManager::LoadRecipeProduct(QString recipeName)
{
    recipeProduct.listRecipe.clear();
    ...
    if ("" != recipeName)
    {
        recipeProduct.curProduct = recipeName;
    }
    ...
    for (int i = 0; i < matrixList.count(); i++) {
        ...
        matrix.productName = matrixNode.attribute("productName");
        matrix.recipeMotor = matrixNode.attribute("recipeMotor");
        matrix.recipeTray = matrixNode.attribute("recipeTray");
        matrix.recipeElectricSaw = matrixNode.attribute("recipeElectricSaw");
        matrix.recipeAirtight = matrixNode.attribute("recipeAirtight");
        recipeProduct.listRecipe.push_back(matrix);
    }

    for (int i = 0; i < recipeProduct.listPlatformMatrix.size(); i++) {
        MatrixSetting& matrix = recipeProduct.listPlatformMatrix[i];
        if (recipeProduct.curProduct == matrix.productShow) {
            for (int j = 0; j < recipeProduct.listRecipe.size(); j++) {
                ProductMatrix& matrixProduct = recipeProduct.listRecipe[j];
                if (matrixProduct.productName == matrix.productName) {
                    recipeMotor.curRecipe = matrixProduct.recipeMotor;
                    recipeTray.curRecipe = matrixProduct.recipeTray;
                    recipeElectricSaw.curRecipe = matrixProduct.recipeElectricSaw;
                    recipeAirtight.curRecipe = matrixProduct.recipeAirtight;
...
bool ParamManager::SaveRecipeProduct()
{
    ...
    for (int i = 0; i < recipeProduct.listRecipe.size(); i++) {
        ProductMatrix& matrix = recipeProduct.listRecipe[i];
        QDomElement matrixNode = domDocument.createElement("product");
        matrixNode.setAttribute("productName", matrix.productName);
        matrixNode.setAttribute("recipeMotor", matrix.recipeMotor);
        matrixNode.setAttribute("recipeTray", matrix.recipeTray);
        matrixNode.setAttribute("recipeAirtight", matrix.recipeAirtight);
        matrixNode.setAttribute("recipeElectricSaw", matrix.recipeElectricSaw);
        root.appendChild(matrixNode);
    }
    QFile inFile(fileName);
...
bool ParamManager::LoadRecipeList(QString filename, QStringList& listValue)
{
    listValue.clear();
    QFile inFile(filename);
    if (!inFile.exists()) return true;
...
void ParamManager::SetMachineStatus(MachineRunStatus status)
{
    devStatus=status;
    VisAppBus::sendEvent("UpdateDevStatus",status);
