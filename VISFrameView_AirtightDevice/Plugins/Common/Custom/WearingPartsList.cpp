        ...
        if (2 == partKeys[i].split("@").size())
        {
            if (LIMITFREQUENCY == partKeys[i].split("@").last().toInt())
            {
					...
					(station != -1 && partKeys[i].contains(textModule)))
                {
                    ...
                    {
                        CMsgBox::showFatal(this,QStringLiteral("警告"), QStringLiteral("%1已到使用寿命").arg(partKeys[i].split("@").first()),CMsgBox::Ok);
                    }
                }
            }
            else
            {
                ...
                {
                    CMsgBox::showFatal(this,QStringLiteral("警告"), QStringLiteral("请检修：%1").arg(partKeys[i].split("@").first()),CMsgBox::Ok);
                }
            }
        }
    }
...
void WearingPartsList::showEvent(QShowEvent *event)
{
    ...
    for (int i = 0; i < partKeys.size(); i++)
    {
        ...
        if (part){}
            //Common::instance()->CreateParamsChangeRecord(part);
    }
    delete settings;
    settings = nullptr;
}
...
void WearingPartsList::AddPart()
{
    ...
    if (station.isEmpty())
    {
        CMsgBox::showWarning(this,QStringLiteral("警告"), QStringLiteral("输入为空，不予添加"),CMsgBox::Ok);
        return ;
    }
    QString moudleName;
    ...
    {
        //报警，输入名称为空
        CMsgBox::showWarning(this,QStringLiteral("警告"), QStringLiteral("取消操作"),CMsgBox::Ok);
        return ;
    }
    ...
    if (QString::fromLocal8Bit("限制使用次数") == type)
    {
        ...
        {
            CMsgBox::showWarning(this,QStringLiteral("警告"), QStringLiteral("%1维护/保养件已经存在").arg(name),CMsgBox::Ok);
            delete settings;
            settings = nullptr;
            return ;
        }
    ...
    CMsgBox::showSuccess(this,QStringLiteral("警告"), QStringLiteral("%1维护/保养件已经存在").arg(name),CMsgBox::Ok);
}
...
void WearingPartsList::DeletePart()
{
    ...
    CMsgBox::showWarning(this,QStringLiteral("警告"), QStringLiteral("删除成功"),CMsgBox::Ok);
}
